...
export default function ProductList({ data, productClusters, filterProductNames }: Props) {
    ...
    <>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {data
            .filter(
              item => !filterProductNames.length || filterProductNames.includes(item.product_name)
            )
            .map((item, idx) => {
              return (
                <tr key={idx}>
